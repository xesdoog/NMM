...
bool Pointers::Init()
{
    ...
    */
    ...
    scanner.Add(life_support_ptrn, [this](PointerCalculator ptr) {
		LifeSupportPatch = NopInstr(ptr.As<std::uint8_t*>());
    });
    ...
    scanner.Add(jetpack_fuel_ptrn, [this](PointerCalculator ptr) {
		JetPackFuelPatch = NopInstr(ptr.Add(9).As<std::uint8_t*>());
    });
    ...
    scanner.Add(jetpack_fuel_ptrn2, [this](PointerCalculator ptr) {
		JetPackFuelPatch2 = NopInstr(ptr.As<std::uint8_t*>());
    });

    constexpr auto jetpack_pwr_ptrn = Pattern<"F3 0F 10 ?? ?? ?? ?? ?? 0F 10 87 ?? ?? 00 00 48 8D">("JetPackPower");
	scanner.Add(jetpack_pwr_ptrn, [this](PointerCalculator ptr) {
		JetpackPowerPatch = CavePatches::Add(ptr.As<std::uint8_t*>(),
            std::to_array<uint8_t>({ 0xB8, 0x00, 0x00, 0xC8, 0x42, 0x66, 0x0F, 0x6E, 0xD0 }));
    });

    constexpr auto jetpack_pwr_ptrn1 = Pattern<"EB ?? F3 0F 10 05 ?? ?? ?? ?? F3 0F 11 81 ?? ?? 00 00 F3 0F 10 0D">("JetPackPower1");
	scanner.Add(jetpack_pwr_ptrn1, [this](PointerCalculator ptr) {
		JetpackPowerPatch1 = CavePatches::Add(ptr.Add(2).As<std::uint8_t*>(),
		    std::to_array<uint8_t>({0xB8, 0x00, 0x00, 0xC8, 0x42, 0x66, 0x0F, 0x6E, 0xC0}));
    });

    constexpr auto jetpack_pwr_ptrn2 = Pattern<"F3 0F 10 0D ?? ?? ?? ?? F3 0F 11 89 ?? ?? 00 00 F3 0F 10 05 ?? ?? ?? ?? 48 8B 9C">("JetPackPower2");
	scanner.Add(jetpack_pwr_ptrn2, [this](PointerCalculator ptr) {
		JetpackPowerPatch2 = CavePatches::Add(ptr.As<std::uint8_t*>(),
		    std::to_array<uint8_t>({0xB8, 0x00, 0x00, 0xC8, 0x42, 0x66, 0x0F, 0x6E, 0xC8}));
    });
    ...
    scanner.Add(health_ptrn, [this](PointerCalculator ptr) {
        PlayerHealthPatch = NopInstr(ptr.As<std::uint8_t*>());
    });
    ...
    scanner.Add(stamina_ptrn, [this](PointerCalculator ptr) {
		PlayerStaminaPatch = NopInstr(ptr.Add(5).As<std::uint8_t*>());
    });
    ...
    scanner.Add(env_protection_ptrn, [this](PointerCalculator ptr) {
		EnvProtectionPatch = NopInstr(ptr.Add(7).As<std::uint8_t*>());
    });
    ...
    scanner.Add(exosuit_shield_ptrn, [this](PointerCalculator ptr) {
		ExosuitShieldsPatch = NopInstr(ptr.As<uint8_t*>());
        /*
    ...
    scanner.Add(free_crafting_ptrn, [this](PointerCalculator ptr) {
		FreeCraftingPatch = NopInstr(ptr.As<std::uint8_t*>());
    });
    ...
    scanner.Add(launch_thruster_ptrn, [this](PointerCalculator ptr) {
		LaunchThrusterPatch = NopInstr(ptr.As<uint8_t*>());
    });
    ...
    scanner.Add(pulse_engine_ptrn, [this](PointerCalculator ptr) {
		PulseEnginePatch = NopInstr(ptr.Add(5).As<uint8_t*>());
    });
    ...
    scanner.Add(hyper_drive_ptrn, [this](PointerCalculator ptr) {
		HyperDrivePatch = NopInstr(ptr.As<uint8_t*>());
    });
    ...
    scanner.Add(ship_laser_ptrn, [this](PointerCalculator ptr) {
		ShipLaserPatch = NopInstr(ptr.Add(4).As<uint8_t*>());
    });
    ...
    scanner.Add(inf_mining_ptrn, [this](PointerCalculator ptr) {
		InfMiningBeamPatch = NopInstr(ptr.As<std::uint8_t*>());
    });
    ...
    scanner.Add(inf_mining_ptrn2, [this](PointerCalculator ptr) {
		InfMiningBeamPatch2 = NopInstr(ptr.Add(8).As<std::uint8_t*>());
    });

    constexpr auto inf_terrain_ptrn = Pattern<"?? 0F 2C ?? 2B ?? 85 ?? 0F 4E ?? 89 4F ??">("TerrainManipulator");
    scanner.Add(inf_terrain_ptrn, [this](PointerCalculator ptr) {
		InfTerrainToolPatch = NopInstr(ptr.Add(4).As<std::uint8_t*>());
    });

    constexpr auto inf_grenades_ptrn = Pattern<"33 ?? 48 8D 55 B0 48 8D 8D ?? ?? ?? ?? 66 89 75 BD">("Grenades");
    ...
    constexpr auto inf_bj_ammo_ptrn = Pattern<"89 46 18 48 8B 95 ?? ?? ?? ?? 4C 8D 4D ??">("BlazeJavelineAmmo");
    ...
    constexpr auto inf_nc_ammo_ptrn = Pattern<"89 46 18 49 8B 06 49 8B ?? 41 88 BE ?? ?? ?? ?? 45 89 BE">("NeuronCannonAmmo");
    ...
    constexpr auto mtt_dmg_ptrn = Pattern<"89 7B 50 ?? 43 48 01 48 8B 4E ?? 8B 49 ?? ??">("MultitoolDamage");
...
bool Pointers::LateInit() 
{
	...
	const auto nms = mmgr.Get("NMS.exe");
	auto late_scanner = PatternScanner(nms);
	constexpr auto currency_ptrn = Pattern<"8B 88 ?? B8 00 00 89 8F 10 2F 00 00 45 33 ?? 45 0F 57 ?? 39 B7">("Currency");
	late_scanner.Add(currency_ptrn, [this](PointerCalculator ptr) {
		CurrencyInstruction = ptr.As<void*>();
	});

    if (!late_scanner.Scan())
    {
        LOG(WARNING) << "Some late patterns could not be found, some features may not work.";
        return false;
	}
